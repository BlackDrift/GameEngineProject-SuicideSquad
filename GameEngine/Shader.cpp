...
void Shader::InitializeShader(ID3D12Device* device, std::string name)
{
	...
	if (name == "default") {
		...
		m_HResult = CompileShaderS(L".default.hlsl", "vs_main", "vs_5_0", &m_VSByteCode);
		if (m_HResult != S_OK)
		{
		...
		m_HResult = CompileShaderS(L".default.hlsl", "ps_main", "ps_5_0", &m_PSByteCode);
		if (m_HResult != S_OK)
		{
			OutputDebugString("Error Compiling Shader");
			return;
		...
		{
			OutputDebugString("Error Initializing RootSignature");
			return;
		}
	...
	{
		OutputDebugString("Error Initializing PipelineState");
		return;
	}
}
